...
class PlannerCrew:
    ...
    def planner_crew(self) -> Crew:
        ...
        if self.crew_instance is None:
            self.crew_instance = Crew(
                agents=[self.planner_agent()],  # Fixed: use list of agent instances
                tasks=[self.planner_task()],    # Fixed: use list of task instances
                process=Process.sequential,
                verbose=True,
                memory=False,
            )
            logger.info("Crew instance created successfully")
        logger.info("Crew instance created successfully1")
        return self.crew_instance
