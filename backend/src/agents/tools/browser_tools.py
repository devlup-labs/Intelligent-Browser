...
class InstagramLoginInput(BaseModel):
    parsed_html: Optional[str] = Field(
        None, description="Parsed HTML content of the current page"
    )
    page: Optional[Page] = Field(
        None, description="The current Playwright Page object"
    )
...
class InstagramLoginInput(BaseModel):
    ...
    class Config:
        arbitrary_types_allowed = True  # allow Playwright Page to be passed

# ----------------- TOOL DEFINITION -----------------
class InstagramLoginTool(BaseTool):
    name: str = "instagram_login_tool"
    description: str = "Logs into Instagram on the current active page."
    args_schema: Type[BaseModel] = InstagramLoginInput

    async def _arun(
        self, parsed_html: Optional[str] = None, page: Optional[Page] = None
    ) -> dict:
        try:
            if not page:
                return {"success": False, "error": "No active Playwright page provided"}

            # ✅ Use existing page (do not open a new browser)
            await page.fill('input[name="username"]', "Intelli_Browse")
            await page.fill('input[name="password"]', "wiggly.213")
            await page.click('button[type="submit"]')

            return {"success": True, "message": "Instagram login attempted"}
        except Exception as e:
    ...
    def _run(
        ...
        """
        CrewAI requires `_run` (sync version). Since our tool is async-only,
        this just raises an error so the agent never calls it in sync mode.
        """
        raise NotImplementedError("InstagramLoginTool only supports async execution")
